...
class ExampleManager : public nodelet::Nodelet {
...
private:
  ...
  std::shared_ptr<example_manager::CommonHandlers_t> common_handlers_;

  // | --------------- dynamic loading of plugins --------------- |

  std::unique_ptr<pluginlib::ClassLoader<example_manager::Plugin>> plugin_loader_;  // pluginlib loader
  std::vector<std::string>                                         _plugin_names_;
  std::map<std::string, PluginParams>                              plugins_;      // map between plugin names and plugin params
  std::vector<boost::shared_ptr<example_manager::Plugin>>          plugin_list_;  // list of plugins, routines are callable from this
...
void ExampleManager::onInit() {

  ros::NodeHandle nh_ = nodelet::Nodelet::getMTPrivateNodeHandle();

  ros::Time::waitForValid();

  ROS_INFO("[ExampleManager]: initializing");

  // --------------------------------------------------------------
  // |                           params                           |
  // --------------------------------------------------------------

  mrs_lib::ParamLoader param_loader(nh_, "ExampleManager");
  ...
  common_handlers_ = std::make_shared<example_manager::CommonHandlers_t>();

  common_handlers_->some_shared_object = std::make_shared<std::string>(example_of_a_shared_object_);

  common_handlers_->vector_calculator.vectorNorm = boost::bind(&ExampleManager::vectorNorm, this, _1);
  ...
  plugin_loader_ = std::make_unique<pluginlib::ClassLoader<example_manager::Plugin>>("example_manager", "example_manager::Plugin");
  ...
  for (int i = 0; i < int(_plugin_names_.size()); i++) {
    ...
    try {
      ROS_INFO("[ExampleManager]: loading the plugin '%s'", new_plugin.address.c_str());
      plugin_list_.push_back(plugin_loader_->createInstance(new_plugin.address.c_str()));
    }
    catch (pluginlib::CreateClassException& ex1) {
      ROS_ERROR("[ExampleManager]: CreateClassException for the plugin '%s'", new_plugin.address.c_str());
      ROS_ERROR("[ExampleManager]: Error: %s", ex1.what());
      ros::shutdown();
    }
    catch (pluginlib::PluginlibException& ex) {
      ROS_ERROR("[ExampleManager]: PluginlibException for the plugin '%s'", new_plugin.address.c_str());
      ROS_ERROR("[ExampleManager]: Error: %s", ex.what());
      ros::shutdown();
    }
  }

  ROS_INFO("[ExampleManager]: plugins were loaded");
  ...
  for (int i = 0; i < int(plugin_list_.size()); i++) {
    try {
      ...
      ROS_INFO("[ExampleManager]: initializing the plugin '%s'", it->second.address.c_str());
      plugin_list_[i]->initialize(nh_, _plugin_names_[i], it->second.name_space, common_handlers_);
    }
    catch (std::runtime_error& ex) {
      ROS_ERROR("[ExampleManager]: exception caught during plugin initialization: '%s'", ex.what());
    }
  }

  ROS_INFO("[ExampleManager]: plugins were initialized");
...

  {
    ...
    if (!check) {
      ROS_ERROR("[ExampleManager]: the initial plugin (%s) is not within the loaded plugins", _initial_plugin_name_.c_str());
      ros::shutdown();
    }
  }
  ...
  ROS_INFO("[ExampleManager]: activating plugin with idx %d on the list (named: %s)", _initial_plugin_idx_, _plugin_names_[_initial_plugin_idx_].c_str());
  ...
  timer_update_ = nh_.createTimer(ros::Rate(_rate_timer_update_), &ExampleManager::timerUpdate, this);

  // | ----------------------- finish init ---------------------- |

  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[ExampleManager]: could not load all parameters!");
    ros::shutdown();
  }

  is_initialized_ = true;

  ROS_INFO("[ExampleManager]: initialized");
}
...
void ExampleManager::timerUpdate([[maybe_unused]] const ros::TimerEvent& event) {
  ...
  if (result) {
    ...
    // print the result
    ROS_INFO("[ExampleManager]: plugin update() returned: %.2f", result.value());

  } else {

    ROS_ERROR("[ExampleManager]: plugin update failed!");
  }
}
...
double ExampleManager::vectorNorm(const Eigen::Vector3d& input) {

  ROS_INFO("[ExampleManager]: somebody called my vectorNorm() function, probably some plugin");
...
PLUGINLIB_EXPORT_CLASS(example_manager::ExampleManager, nodelet::Nodelet)
